				 ...
				 char *end)
{
	...
	if (clen > 7 && os_strncmp(custom, "wpa_ie=", 7) == 0) {
		...
		spos = custom + 8;
		bytes = custom + clen - spos;
		wpa_printf(MSG_DEBUG, "get a wapi_ie, bytes = %d...\n", bytes);
		if (bytes & 1)
		{
					   ...
					   struct wext_scan_data *data)
{
	...
	wpa_printf(MSG_DEBUG, "%s: wpai_ie_len=%d\n", __FUNCTION__, r->wapi_ie_len);
#endif
	pos = (u8 *) (r + 1);
	if (ssid_ie == NULL) {
		/*
				       ...
				       const u8 *key, size_t key_len)
{
	...
	if ( (alg == WAPI_ALG_SMS4) && (seq_len > IW_ENCODE_SEQ_MAX_SIZE * 2) ) {
		wpa_printf(MSG_DEBUG, "%s: Invalid seq_len for WAPI_ALG_SMS4 %lu", __FUNCTION__, (unsigned long) seq_len);
		return -1;
	}
