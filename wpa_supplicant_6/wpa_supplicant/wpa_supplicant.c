...
static const char *wapi_cert_dir = "/data/misc/wifi/cert/";
...
struct wapi_config * wapi_config_init(struct wpa_ssid *ssid)
{
	...
	/* cert_name is in the same path as wapi_user_cert, but with the ssid prepended in 
	front of the wapi_user_cert filename */
	if (config->wapi_policy & 0x08) {
...
#if 1
			/* create merged cert in specific directory, otherwise
			   the cert file will mix together with user cert */
			strcpy(ssid->cert_name, wapi_cert_dir); 
			ptr = strrchr(ssid->wapi_user_cert, '/');
			if (ptr==NULL) { /* it doesn't have a '/' so it only has a filename but no path */
				ptr = ssid->wapi_user_cert;
			}
			else {
				ptr = strrchr(ssid->wapi_user_cert, '/')+1;
			}
			to = ssid->cert_name+strlen(ssid->cert_name);
#else
			strcpy(ssid->cert_name, ssid->wapi_user_cert); 
			ptr = strrchr(ssid->cert_name, '/');
			if (ptr==NULL) { /* it doesn't have a '/' so it only has a filename but no path */
				to = ssid->cert_name;
				ptr = ssid->wapi_user_cert;
			}
...
#endif
			for (i=0; i<ssid->ssid_len; i++, to++) {
				*to = ssid->ssid[i];
			}
			strcpy(to, ptr); /* append the user_cert filename without its original path */
	...
	if (config->wapi_policy & WAPI_KEY_MGMT_PSK) { /*bit 2:PSK*/
		...
		if (ssid->passphrase != NULL) {
			wpa_printf(MSG_ERROR, "%s: ssid->passphrase !== NULL, took ssid->passphrase %s as wapi psk\n",
			...
			wpa_printf(MSG_ERROR, "%s: ssid->passphrase == NULL, took ssid->psk %s as wapi psk\n", 
			      ...
			      struct wpa_scan_res *bss, struct wpa_ssid *ssid)
{
	...
	if ( (ssid->wapi) || (ssid->proto & WPA_PROTO_WAPI) || (ssid->key_mgmt == WPA_KEY_MGMT_WAPI_PSK ) || (ssid->key_mgmt == WPA_KEY_MGMT_WAPI_CERT ) ) { // associating to a WAPI network
		...
		wpa_printf(MSG_ERROR, "WAPI ssid->wapi %d ssid->proto %d ssid->key_mgmt %d", ssid->wapi,
