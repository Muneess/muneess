...
gboolean a2dp_sep_lock(struct a2dp_sep *sep, struct avdtp *session)
{
    ...
    DBG("[a2dp_sep_lock] SEP %p locked", sep->sep);  //[cellcore][mingyan] debug.
	sep->locked = TRUE;
...
gboolean a2dp_sep_unlock(struct a2dp_sep *sep, struct avdtp *session)
{
    ...
    //[+][cellcore][mingyan] device fast reboot while unpair BT-HS
    if (sep == NULL) {
        DBG("[a2dp_sep_unlock] sep is null.");
        return FALSE;
    }
    if (sep->sep == NULL) {
        DBG("[a2dp_sep_unlock] sep->sep is null.");
        return FALSE;
    }
    //[-]
